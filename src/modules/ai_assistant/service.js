...
const processChat = async (userMessage, userId, sessionId, authToken, allowedModules) => {
  try {
    ...
    if (Array.isArray(allowedModules)) {
      ...
      systemPrompt += `\n\n*** STRICT ACCESS CONTROL ***\nUser Rights: The user ONLY has access to the following modules: [${modulesList}].\nYou are PROHIBITED from providing any data, information, or assistance related to modules NOT in this list.\n\nIf the user's request involves a restricted module (e.g., asking for 'Quotation' data when 'Quotation' is not listed above, or 'Worker' when 'HR' is not listed), you must REFUSE immediately.\n\nRefusal Response:\n"Mohon maaf, Anda tidak memiliki hak akses untuk module tersebut."\n\nDo not explain why (other than the standard refusal). Do not apologize profusely. Do not offer alternatives. STOP and return the refusal response.`;
    }
